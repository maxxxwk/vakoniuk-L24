...
class PostValidationUseCase @Inject constructor(){
    operator fun invoke(postEntity: PostEntity) = when {
        ...
        postEntity.title.toLowerCase().split(" ").contains("реклама") -> PostValidationResult.INCORRECT_TITLE
        postEntity.title.toLowerCase().split(" ").contains("товар") -> PostValidationResult.INCORRECT_TITLE
        postEntity.title.toLowerCase().split(" ").contains("куплю") -> PostValidationResult.INCORRECT_TITLE
